...
export class AddUserDeletedEnumTypeForNotificationEntity1694567494148 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `CREATE TYPE "public"."notification_type_enum" AS ENUM('info', 'user.created', 'user.confirmed', 'user.deleted', 'admin.created', 'order.created', 'order.cancelled', 'order.status.updated', 'request.created', 'request.rejected', 'request.approved')`,
    );
    await queryRunner.query(
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `CREATE TYPE "public"."notification_type_enum_old" AS ENUM('info', 'user.created', 'user.confirmed', 'admin.created', 'order.created', 'order.cancelled', 'order.status.updated', 'request.created', 'request.rejected', 'request.approved')`,
    );
    await queryRunner.query(
      `ALTER TABLE "notification" ALTER COLUMN "type" DROP DEFAULT`,
    );
